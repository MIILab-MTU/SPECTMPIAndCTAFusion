...
% Robust end-to-end script (txt/ply I/O, dedup, jitter, safe normalization)
%********************
close all; clc; clear;
...
%% ========= 配置 =========
% 支持 .txt / .csv / .tsv（每行 x y z，允许多余列与注释 # % //）或 .ply
% src_path = "../data/src_points.txt";   % 如需 .ply: "../data/tr_reg_059.ply"
% tgt_path = "../data/tgt_points.txt";   % 如需 .ply: "../data/tr_reg_057.ply"
patientname = 'zengyuming';
src_path =   sprintf('E:/proj/peizhun/file_process/cloud_result0507/%s/cu.txt',patientname); % 也可改成 "../data/tr_reg_059.ply"
tgt_path = sprintf('F:/cta-spect/CTA-SPECT/data_process/cta/final/%s/ijkcta.txt',patientname);   % 也可改成 "../data/tr_reg_057.ply"

gridStep      = 0.03;   % 下采样体素（过稀→调小，如 0.01/0.005）
dedup_tol     = 1e-8;   % 去重公差（按你数据量级适当调到 1e-7/1e-6）
jitter_ratio  = 1e-6;   % 抖动强度（若仍 NaN，试 1e-5）
force_cpu     = false;  % 设为 true 可强制 CPU 配准以对比稳定性
