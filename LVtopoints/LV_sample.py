...
def LV_main(patient, cta_path):
    ...
    for i in os.listdir(cta_path):
        ...
        if patient_name in s:
            for j in os.listdir(os.path.join(cta_path, i,r'mySeg')):
                if 'LV.stl' in j:
                    stl_path = rf'{cta_path}\{i}\mySeg\{j}'
    print(stl_path)


    # p_name = patient_name.replace(" ", "")
    output_txt = rf'LVtopoints/name/{patient_name}/{patient_name}_LV_transformed.txt'

    if not os.path.exists(rf'LVtopoints/name/{patient_name}'):
        os.makedirs(rf'LVtopoints/name/{patient_name}')
